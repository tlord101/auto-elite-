...
export const verifyAdminAccess = async (uid: string): Promise<AdminAccess> => {
  const adminRef = doc(db, 'admins', uid);
  const adminSnap = await getDoc(adminRef);
  if (!adminSnap.exists()) {
    return { hasAccess: false, role: 'editor' };
  }

  const data = adminSnap.data();
  const role = (data?.role as AdminAccess['role']) || 'editor';
...
export const subscribeAdminFinancing = (onData: (requests: FinancingRequest[]) => void) => {
  ...
  return onSnapshot(
    ...
    (error) => {
      console.error('Failed to subscribe financing requests', error);
      onData([]);
    }
  );
};
